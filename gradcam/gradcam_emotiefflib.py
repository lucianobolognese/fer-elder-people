"""
Grad-CAM per EmotiEffLib (EfficientNet B2 - 7 Classi)
======================================================
Genera heatmap di attenzione per visualizzare il bias morfologico.
Include il modulo ReWired per ricomporre il modello completo.
"""
...
OUTPUT_DIR = r"C:\Users\ciano\Desktop\Tesi\GradCAM_Output_EmotiEffLib"
N_SAMPLES_PER_CLASS = 3


EMOTIEFF_CLASSES = ['anger', 'disgust', 'fear', 'happy', 'neutral', 'sad', 'surprise']
...
class ReWiredEmotiEff(torch.nn.Module):
    """
    EmotiEffLib separa il feature_extractor (PyTorch) dal classificatore (Numpy).
    Questo wrapper li salda insieme in un unico modello PyTorch end-to-end,
    indispensabile per far calcolare i gradienti alla Grad-CAM.
    """
...
def generate_bias_comparison_figure(gradcam, device, emotion_class, image_paths, output_path, true_class_idx):
    ...
    fig.suptitle(f"Grad-CAM (EmotiEffLib) — Classe: '{emotion_class.upper()}' — SAGE-Face",
...
def main():
    ...
    print("Inizializzazione EmotiEffLib (enet_b2_7)...")
    from emotiefflib.facial_analysis import EmotiEffLibRecognizer
    fer = EmotiEffLibRecognizer(engine="torch", model_name="enet_b2_7", device=device)
    

    model = ReWiredEmotiEff(fer, device)
    model.eval()

    TARGET_LAYER_NAME = 'features_extractor.blocks.6' 
    # TARGET_LAYER_NAME = 'features_extractor.conv_head'
    ...
    for emotion_name, true_idx in FOLDER_TO_INDEX.items():
        ...
        out_path = os.path.join(OUTPUT_DIR, f"gradcam_comparison_{emotion_name}.png")
    ...
    print("\n=== Generazione figure singole (Cap. 1.3) ===")
    for key, (folder_or_path, true_idx, title) in EXAMPLE_IMAGES.items():
        ...
        out_path = os.path.join(OUTPUT_DIR, f"gradcam_single_{key}.png")
        print(f"  '{key}': {img_path}")
        generate_single_gradcam(gradcam, device, img_path, out_path, true_idx, title)

    gradcam.remove_hooks()
    print(f"\nDone. Tutte le figure per EmotiEffLib salvate in: {OUTPUT_DIR}")
